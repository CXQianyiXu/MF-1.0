...
errors = {
    ...
    'E0002': '\nIllegal characters.',
    'E0003': '\nNo unknowns.',
    'E0004': '\nWrong number of characters between spaces.',
    'E0005': '\nNon breaking space.',
    'E0006': '\nWrong number of division sign in the term.',
    'E0007': '\nThe first term only has a minus sign.',
    'E0008': '\nThere are some operators in terms.',
...
def prepare():
    print(
        'Please write a system of linear equations.\n'
        'Put a space between the term and the operator.\n'
        'Do not put irrationals. Put fractions before the unknowns.\n'
        'Do not use decimals.\n'
        'You can only use the minus sign before the first terms.\n'
...
def check_strings_between_spaces(equation: str):
    ...
    if not len(equation_list) % 2:
        error('E0004')
    for terms_or_operators in equation_list:
        if not terms_or_operators:
            error('E0005')
...
def check_char_beside_division(char: str, string: str):
    if char == '/' and (True if (string.index('/') + 1 == len(string))
                        else ((string[string.index('/') + 1] not in numbers[1:])
                              or (not string.index('/'))
                              or (string[string.index('/') - 1] not in numbers))):
        error('E0009')
...
def check_first_terms(string_term: str):
    if string_term == '-':
        error('E0007')
    check_terms(string_term[(string_term.find('-') + 1):])
        ...
        for string in equation:
            if string in operators:
                ...
                if is_first_term:
                    ...
                    string_term = string_term + string
...
def solve_equations(sys_equ_list: list, unknowns_list: list):
    ...
    for i in range(len(unknowns_list)):
        for j in range(i):
            sys_equ_list[-i - 1][1] = minus_two_string(sys_equ_list[-i - 1][1], multiple(sys_equ_list[-i - 1][0].pop(),
                                                                                         equation_list[-j - 1]))
...
if __name__ == '__main__':
    ...
    unknowns = check_number_equations_unknowns(equations, unknowns)  # list[str], list[str]
